...
def workspace_tool_policy() -> str:
    return section(
        "Tool Policy",
        [
            "Use file tools inside the virtual project root `/workspace`; declared requirement references may also be read from their exact read-only `/references/...` paths.",
            ...
            "Reference catalog entries expose provenance and availability without file contents. Read relevant available references with `read_file` before finalizing artifacts, use offsets for long files, and never try to edit, create, execute, broadly enumerate, or discover undeclared paths under `/references`.",
            "Unavailable references are non-blocking evidence gaps: do not invent their contents, and identify the missing evidence in the stage summary or implementation handoff.",
            "Reference ancestry and declaration order do not imply priority. Follow explicit precedence stated in requirements or references; if none resolves a conflict, record the interpretation chosen for the artifact.",
            ...
            "Shell execution is available through the built-in `execute` tool. Run commands only from `/workspace` or a subdirectory, and never use shell execution for `/references` or host paths outside `/workspace`.",
...
def code_task_exploration_policy() -> str:
    return section(
        ...
        [
            ...
            "If a target source file is already large, roughly over 500 lines, avoid adding another feature-sized block to it. Prefer extracting a cohesive component, hook, API client, service, repository, or route module that connects back through the existing app structure.",
            "When editing a large file is unavoidable, make a narrow connector edit there and place new behavior in smaller owned modules.",
