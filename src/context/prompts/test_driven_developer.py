        ...
        [
            ...
            section(
                ...
                [
                    ...
                    "When context includes a `permission_contract`, enforce it at the server/API/FUNC boundary before returning protected data or committing state. Frontend visibility checks are not authorization enforcement.",
                    "For HTTP authorization, keep unauthenticated and authenticated-forbidden behavior consistent with 401 and 403; for non-HTTP use equivalent explicit errors. Enforce stated owner, supervisor, college, or school data scopes in the backend query/mutation path.",
                    "Honor scenario/step actors and implement only state transitions supported by the requirement description or scenarios; reject stated wrong-actor, out-of-scope, or invalid transitions before mutation.",
                ],
            ),
            section(
                "Execution Flow",
    ...
    sections.append(
        section(
            ...
            [
                ...
                "If dynamic context contains a permission contract, implement the allowed, forbidden, anonymous, and data-scope behavior exercised by the tests at the owned backend boundary before protected reads or writes. Do not rely on hidden controls.",
                "Use scenario and step actors as the acting identities. When state_flow is present, persist and validate only requirement-supported transitions and reject wrong-role or invalid transitions before changing state.",
