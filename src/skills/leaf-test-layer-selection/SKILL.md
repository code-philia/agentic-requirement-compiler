...
24. Start from the current interface manifest and its exact `file_path` values. Read the owned source file, the direct test runner setup only when needed, and the target test file if it already exists; do not explore unrelated pages, routes, database files, or package manifests.
