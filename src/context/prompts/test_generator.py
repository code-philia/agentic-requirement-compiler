        ...
        [
            ...
            section(
                ...
                [
                    ...
                    "For each declared scenario, generate or extend an E2E test that exercises the user-visible flow and asserted outcome through the hosted app.",
                    "For auth/session scenarios, assert observable global state changes through shared chrome, current-user/session UI, route state, or session API behavior. Do not reduce authenticated-state coverage to a page-local success banner.",
    ...
    sections.append(
        section(
            ...
            [
                ...
                "Scenario-driven E2E tests should follow the scenario flow: set up the necessary state, perform the user actions, and assert the scenario outcome through visible UI or backend-hosted app behavior.",
                "If the scenario changes authentication state, tests should check that the shared app state reflects the transition, such as public header controls changing to current-user/account state, protected/public navigation updating, or `/api/auth/session` returning the expected user/session after the action.",
                "For E2E tests, choose selectors from requirement-stated user-facing labels, roles, routes, and outcomes first. If the requirement does not specify exact selectors, define stable accessible selectors in the test as part of the executable contract so implementation can align to them.",
