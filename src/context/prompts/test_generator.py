        ...
        [
            ...
            section(
                ...
                [
                    ...
                    "When a leaf requirement has a `permission_contract`, test an allowed role, an authenticated denied role when one is declared, and anonymous access according to the description/scenarios. UI visibility checks may supplement but never replace backend rejection coverage.",
                    "When the current contract owns an API or FUNC authorization boundary, include Integration/API coverage that calls it directly. For HTTP distinguish unauthenticated 401 from authenticated-forbidden 403; for non-HTTP assert equivalent explicit errors.",
                    "Preserve scenario-level and step-level actors in setup/actions, and cover state transitions only when the description or scenarios define them; include forbidden or wrong-actor transitions when required.",
    ...
    sections.append(
        section(
            ...
            [
                ...
                "If a permission contract is present, build the role/anonymous matrix from that contract. Exercise the owned backend boundary directly for allowed, authenticated-forbidden, and unauthenticated cases when applicable; a hidden button alone is not authorization coverage.",
                "Use each scenario and step `actor` as the identity performing that setup or action. For `state_flow`, assert only transitions supported by the scenario/description and include stated wrong-role or invalid-transition rejection.",
