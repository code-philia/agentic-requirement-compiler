...
15. Explore in ownership order: reused parent/dependency interface, current UI entrypoint, its direct component or hook, current API client, then only the route/service/repository/database scaffold required by the interface chain. Do not inventory unrelated application areas or manifests.
